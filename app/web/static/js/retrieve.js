...
// Function to get legacy messages for backward compatibility
const getLegacyInvalidPinMessages = () => {
  const messages = [];
  if (window.safexLang?.translate) {
    const ruLegacy = window.safexLang.translate(
      "retrieve.errors.legacy.invalidPin",
      window.safexLang.LANG_RU
    );
    const enLegacy = window.safexLang.translate(
      "retrieve.errors.legacy.invalidPin",
      window.safexLang.LANG_EN
    );
    if (ruLegacy) messages.push(ruLegacy);
    if (enLegacy) messages.push(enLegacy);
  }
  return messages;
};
...
const translate = (key) => {
  return window.safexLang?.translate(`retrieve.errors.${key}`) || "";
};

// We need to check against messages in both languages
const getServerInvalidPinMessages = () => {
  const messages = [];

  // Add legacy messages for backward compatibility
  messages.push(...getLegacyInvalidPinMessages());

  // Add current translations from language.js if available
  if (window.safexLang?.translate) {
    // Get translations for both languages from language.js
    const ruTranslation = window.safexLang.translate(
      "retrieve.errors.invalidPin",
      window.safexLang.LANG_RU
    );
    const enTranslation = window.safexLang.translate(
      "retrieve.errors.invalidPin",
      window.safexLang.LANG_EN
    );

    if (ruTranslation) messages.push(ruTranslation);
    if (enTranslation) messages.push(enTranslation);
  }
...
(async () => {
  ...
  const secretIdInput = document.querySelector(
    "#unlock-form input[name='secret_id']"
  );
  ...
  const renderErrorCard = (message, { invalidPin = false } = {}) => {
    ...
    if (invalidPin) {
      ...
      span.textContent = message || translate("defaultError");
    }
  ...
  const decryptAndRender = (card, exportKey) => {
    ...
    const encryptedBase64 =
      downloadButton?.dataset.downloadBase64 ||
      card.dataset.secretPayload ||
      "";
    if (!encryptedBase64) {
      return;
    }
    ...
    try {
      ...
      renderErrorCard(translate("decryptionFailed"));
      return;
    }
    ...
    if (downloadButton) {
      ...
      const sanitizedName =
        originalName.replace(/\.encrypted$/i, "") || originalName;
  ...
  const startOpaqueLogin = async (pin, secretId) => {
    ...
    try {
      ...
      throw new RevealFlowError(translate("initPin"));
    }
    ...
    if (!response.ok || !payload) {
      const message = payload?.error || translate("initPin");
      const normalized = typeof message === "string" ? message.trim() : "";
      const invalidPin =
        normalized && getServerInvalidPinMessages().includes(normalized);
    ...
    if (!sessionId || !serverResponse) {
      throw new RevealFlowError(translate("serverResponse"));
    }
  ...
  const revealSecret = async (pin) => {
    ...
    if (!secretId) {
      throw new RevealFlowError(translate("noSecretId"));
    }
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (error instanceof RevealFlowError) {
        ...
        renderErrorCard(translate("requestFailed"));
      }
    } finally {
      setLoading(false);
    }
  ...
  const downloadFromButton = (button) => {
    ...
    const contentType =
      button.dataset.downloadType || "application/octet-stream";
    if (!base64) {
      return;
    }
